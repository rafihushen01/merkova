...
const SellerHomeBanner = () => {
  ...
  const fetchbanners = async () => {
    const res = await axios.get(`${serverurl}/sellerhomebanner`);
    setbanners(res.data || []);
  };
  ...
  const submit = async () => {
    ...
    if (selected) {
      await axios.put(
        `${serverurl}/sellerhomebanner/add-images/${selected._id}`,
        fd
      );
    } else {
      await axios.post(`${serverurl}/sellerhomebanner/create`, fd);
    }
    ...
    <div className="min-h-screen text-slate-200">
      ...
      <main className="relative z-10 max-w-[1700px] mx-auto p-10">
        <h1 className="text-5xl font-black italic mb-12">
          MERKOVA <span className="text-blue-500"> Seller HOME BANNERS</span>
        </h1>
            ...
            <AnimatePresence>
              {banners.map((banner) => (
                <BannerCard
                  ...
                  onDelete={async (id) => {
                    await axios.delete(`${serverurl}/sellerhomebanner/${id}`);
                    fetchbanners();
                  }}
                  onReorder={async (data) => {
                    await axios.put(
                      `${serverurl}/sellerhomebanner/reorder/${data._id}`,
                      {
...
export default SellerHomeBanner;
