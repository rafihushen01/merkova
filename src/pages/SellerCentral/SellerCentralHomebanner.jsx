...
const SellerHomebannercard = () => {
  ...
  useEffect(() => {
    axios
      .get(`${serverurl}/sellerhomebanner`)
      .then((res) => {
        setBanners(res.data || []);
        setLoading(false);
      })
  ...
  /* ================= CLICK HANDLER ================= */
  const handleClick = () => {
    if (!banner.navigatelink) return;

    // ✅ INTERNAL LINK
    if (banner.navigatelink.startsWith("/")) {
      navigate(banner.navigatelink);
    } 
    // ✅ EXTERNAL LINK
    else {
      window.open(banner.navigatelink, "_blank");
    }
  };
  ...
  return (
    <section className="relative w-full max-w-[1800px] mx-auto h-[45vh] sm:h-[55vh] lg:h-[65vh] 2xl:h-[75vh] overflow-hidden rounded-[28px] rounded-t-none bg-black shadow-[0_30px_90px_rgba(0,0,0,0.25)] group">
      ...
      {/* ================= GLASSY ARROWS ================= */}
      <button onClick={prevSlide} className="absolute left-4 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-white/15 backdrop-blur-xl flex items-center justify-center text-white cursor-pointer opacity-0 group-hover:opacity-100 transition-all duration-500 hover:bg-white/30 hover:scale-110">
        <ChevronLeft size={26} />
      </button>

      <button onClick={nextSlide} className="absolute right-4 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-white/15 backdrop-blur-xl flex items-center justify-center text-white cursor-pointer opacity-0 group-hover:opacity-100 transition-all duration-500 hover:bg-white/30 hover:scale-110">
...
export default SellerHomebannercard;
